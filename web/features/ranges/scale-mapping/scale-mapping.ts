...
export function normalizedPositionToValue(range: RangeDto, position: number): number {
  const canonicalPosition = range.valueDirection === "descending" ? 1 - position : position;
  return scaleDefinitionNormalizedPositionToValue(range.scaleDefinition, canonicalPosition);
}

export function scaleDefinitionNormalizedPositionToValue(
  definition: ScaleDefinitionDto,
  position: number,
): number {
  if (definition.mode === "linear")
    return definition.start + (definition.end - definition.start) * position;
  if (definition.mode === "logarithmic") {
    ...
    if (definition.detailEmphasis === "low-values")
      return Math.exp(Math.log(definition.start) + logarithmicSpan * position);
    if (position === 0) return definition.start;
    if (position === 1) return definition.end;
    const reflectedValue = Math.exp(Math.log(definition.end) - logarithmicSpan * position);
    return definition.start + definition.end - reflectedValue;
  }
  const segmentIndex = definition.points.findIndex((point) => point.position >= position);
  if (segmentIndex === -1) return definition.points.at(-1)!.value;
  if (segmentIndex <= 0) return definition.points[0].value;
  const previous = definition.points[segmentIndex - 1];
  const next = definition.points[segmentIndex];
  if (!next) return definition.points.at(-1)!.value;
